...
(******************************************************************************)
(*                  This file presents a formalization                        *)
(*              of the axiomatic definition of real numbers                   *)
(*              and verifies their key algebraic properties,                  *)
(*  following Zorich's 'Mathematical Analysis' (Part I, 7th expanded edition) *)
(*                                                                            *)
...
Local Notation "x + y" := fp[[x, y]].
Local Notation "0" := zeroR.
Local Notation "x · y" := fm[[x, y]](at level 40).
Local Notation "1" := oneR.
Local Notation "x ≤ y" := ([x, y] ∈ Leq)(at level 77).
Local Notation "- a" := (∩(\{ λ u, u ∈ R /\ u + a = 0 \})).
Local Notation "x - y" := (x + (-y)).
Local Notation "a ⁻" := (∩(\{ λ u, u ∈ (R ~ [0]) /\ u · a = 1 \}))(at level 5).
Local Notation "m / n" := (m · (n⁻)).
Local Notation "x < y" := (x ≤ y /\ x <> y).


Class Real_Axioms (R_str : Real_Struct) := {
  ...
  Completeness : ∀ X Y, X ⊂ R -> Y ⊂ R -> X <> Φ -> Y <> Φ
    -> (∀ x y, x ∈ X -> y ∈ Y -> x ≤ y)
    -> ∃ c, c ∈ R /\ (∀ x y, x ∈ X -> y ∈ Y -> (x ≤ c /\ c ≤ y)); }.


Variable R_str : Real_Struct.
Variable RA : Real_Axioms R_str.
...
Local Hint Resolve zero_in_R one_in_R one_in_R_Co : real.

(********************* corollaries of the addition axioms *********************)
...
Corollary Plus_Co2 : ∀ x, x ∈ R -> (∃! x0, x0 ∈ R /\ x + x0 = 0).
Proof.
...
(****************** corollaries of the multiplication axioms ******************)
...
(*********** corollaries of the addition and multiplication axioms ************)
...
(********************** corollaries of the order axioms ***********************)
...
(******* corollaries of the order, addition, and multiplication axioms ********)
...
Proof.
  ...
  assert (-x < 0) as []; auto. { apply OrderPM_Co2a; auto. }
Qed.
...
Property Max_Unique : ∀ X c1 c2, Max X c1 -> Max X c2 -> c1 = c2.
Proof.
...
Property Min_Unique : ∀ X c1 c2, Min X c1 -> Min X c2 -> c1 = c2.
Proof.
  intros. destruct H as [H[]],H0 as [H0[]].
  pose proof H1. pose proof H3. apply H2 in H5.
  apply H4 in H6. apply Leq_P2; auto.
...
Property Sup1_equ_Sup2 : ∀ X s, Sup1 X s <-> Sup2 X s.
Proof.
...
Property Inf1_equ_Inf2 : ∀ X i, Inf1 X i <-> Inf2 X i.
Proof.
  split; intros; destruct H.
  - destruct H as [H[]]. repeat split. unfold Included; intros.
    apply AxiomII in H3 as [_[_[]]]; auto. apply AxiomII; split;
...
Theorem SupT : ∀ X, X ⊂ R -> X <> Φ -> (∃ c, Upper X c) -> ∃! s, Sup1 X s.
Proof.
  ...
  exists c. split. apply Sup1_equ_Sup2; auto. intros.
  apply Sup1_equ_Sup2 in H8. apply (Min_Unique Y); auto.
Qed.

Theorem InfT : ∀ X, X ⊂ R -> X <> Φ -> (∃ c, Lower X c) -> ∃! i, Inf1 X i.
Proof.
  ...
  exists c. split. apply Inf1_equ_Inf2; auto. intros.
  apply Inf1_equ_Inf2 in H8. apply (Max_Unique Y); auto.
Qed.
...
Proposition Arch_P10 : ∀ x, x ∈ R -> (∃! k, k ∈ Z /\ k ≤ x /\ x < k + 1).
Proof.
...
Definition Abs := \{\ λ u v, u ∈ R 
  /\ ((0 ≤ u /\ v = u) \/ (u < 0 /\ v = (-u))) \}\.
Local Notation "｜ x ｜" := (Abs[x])(at level 5, x at level 0).

Property Abs_is_Function : Function Abs /\ dom(Abs) = R
  /\ ran(Abs) = \{ λ x, x ∈ R /\ 0 ≤ x \}.
Proof.
...
Property me_zero_Abs : ∀ x, x ∈ R -> 0 ≤ x -> ｜x｜ = x.
Proof.
...
Property le_zero_Abs : ∀ x, x ∈ R -> x ≤ 0 -> ｜x｜ = -x.
Proof.
  intros. destruct Abs_is_Function as [H1[]]. rewrite <-H2 in H.
  apply Property_Value,AxiomII' in H as [_[H[[]|[]]]]; auto.
